...
export function getMessageRepository(): MessageRepository {
  return ENABLE_MOCK_LETTER_SERVICE === true ? new MockMessageRepository() : new DefaultMessageRepository();
}
...
export class DefaultMessageRepository implements MessageRepository {
  ...
  async getPdfByMessageId(messageId: string, userId: string): Promise<PdfDto> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to find pdf. Status: ${response.status}, Status Text: ${response.statusText}`);
    }
