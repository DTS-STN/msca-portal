...
import getPdfByMessageIdJson from '../../resources/cct/get-pdf-by-message-id.json'
...
export function getMessageRepository(): MessageRepository {
  return ENABLE_MOCK_LETTER_SERVICE === true
...
export class DefaultMessageRepository implements MessageRepository {
  ...
  async findMessagesBySin(
    ...
    const response = await httpClient.instrumentedFetch('http.client.interop-api.get-doc-info-by-client-id.gets', 
      url,
      {
        headers: {
    ...
    log.trace('response test' + response)
    ...
    log.trace('Returning letters [%j]', messageDtos)
    return messageDtos
  }
  ...
  async getPdfByMessageId(messageId: string, userId: string): Promise<PdfDto> {
    ...
    const response = await httpClient.instrumentedFetch('http.client.interop-api.get-pdf-by-client-id.gets',
      url,
      {
        headers: {
          'Content-Type': 'application/json',
    ...
    log.trace('Returning pdf [%j]', pdfDto)
...
export class MockMessageRepository implements MessageRepository {
  ...
  async findMessagesBySin(
    ...
    log.info('mock - fetching messages by [%s], [%s]', sin, userId)
  ...
  async getPdfByMessageId(messageId: string): Promise<PdfDto> {
    log.info('mock - pdf by message id ' + messageId)
