...
import { getDashboardCardResponseSchema, getPageAlertResponseSchema, getExitBetaResponseSchema } from '~/schemas/dashboard-service-schemas.server';
...
export function getDashboardService() {
  ...
  async function getDashboardContent(lang: string) {
    ...
    if (response.ok) {
      ...
      const responseDashboardData = responseData.data.schPageV1List.items[0].scFragments.filter((item) => item.scId  === 'dashboard-cards');
      const cardsResponse = getDashboardCardResponseSchema.parse(responseDashboardData[0].scItems);

      const responseExitBetaData = responseData.data.schPageV1List.items[0].scFragments.filter((item) => item.scId  === 'exit-beta-version');
